...
def classify_change(distance_m: float) -> str:
    """Classify a physical distance change into กัดเซาะ (Erosion) / งอก (Accretion) / คงที่ (Stable)."""
...
# ==============================================================================
st.set_page_config(
    page_title="เครื่องมือวิเคราะห์การกัดเซาะชายฝั่ง — Fractal Dimension",
    page_icon="🌊",
    layout="wide",
)

st.title("🌊 เครื่องมือวิเคราะห์การกัดเซาะ/การงอกของชายฝั่ง")
st.caption("วิธี Box-Counting Fractal Dimension — วิเคราะห์และพยากรณ์แนวชายฝั่งจากภาพหลายช่วงเวลา")

with st.expander("ℹ️ เกี่ยวกับเครื่องมือนี้ / หลักการทำงาน", expanded=False):
    st.markdown(
        """
เครื่องมือนี้วิเคราะห์ **ชุดภาพชายฝั่งตามช่วงเวลา** (เช่น ภาพ NDWI จากดาวเทียม
Landsat หรือ Sentinel-2) เพื่อ:

1. สกัด **แนวขอบเขตแผ่นดิน/น้ำ (ชายฝั่ง)** จากแต่ละภาพ ด้วย Otsu Thresholding
   ร่วมกับการดึงขอบแบบ Contour หรือ Canny
2. วัด **ความซับซ้อนเชิงเรขาคณิต** ของแนวชายฝั่งแต่ละภาพ ด้วยค่า
   **Box-Counting Fractal Dimension (FD)** — ค่า FD ยิ่งสูง มักบ่งบอกว่าแนวชายฝั่ง
   มีรูปร่างซับซ้อน/ไม่เรียบมากขึ้น
3. ประมาณ **ระยะการกัดเซาะ/งอกจริง (หน่วยเมตร)** ระหว่างแต่ละช่วงเวลา จากการ
   เปลี่ยนแปลงพื้นที่แผ่นดิน หารด้วยความยาวชายฝั่ง แล้วคูณด้วยค่าความละเอียดภาพ
4. **พยากรณ์** ค่า FD ในอนาคต และแปลงแนวโน้มนั้นเป็นระยะถอยร่น/รุกล้ำของชายฝั่ง
   ที่คาดการณ์ไว้ ตามจำนวนปีล่วงหน้าที่คุณกำหนดเอง

*นี่เป็นการประมาณค่าเชิงสำรวจ/เพื่อการศึกษาเท่านั้น ไม่ใช่การสำรวจชายฝั่งที่ได้รับการรับรอง*
        """
    )
...
land_choice = st.sidebar.radio(
    ...
    index=0,
    help="ขึ้นอยู่กับรูปแบบภาพ NDWI/แบนด์ของคุณ หากผลลัพธ์ดูกลับด้าน ให้สลับตัวเลือกนี้"
)
...
max_dim = st.sidebar.slider(
    ...
    help="ภาพที่ใหญ่กว่าค่านี้จะถูกย่อขนาดเพื่อความเร็ว โดยค่าสเกล (เมตร/พิกเซล) จะถูกปรับอัตโนมัติ "
         "ให้ผลลัพธ์ในหน่วยเมตรยังคงถูกต้อง"
)

st.sidebar.markdown("---")
st.sidebar.caption(
    "เคล็ดลับ: อัปโหลดภาพเรียงตามลำดับเวลาคร่าวๆ แอปจะช่วยเดาปีเริ่มต้นให้อัตโนมัติ "
    "ซึ่งสามารถแก้ไขได้"
)
...
uploaded_files = st.file_uploader(
    "อัปโหลดภาพชายฝั่งได้หลายไฟล์ (NDWI / Landsat / Sentinel-2 หรือภาพแผ่นดิน-น้ำใดๆ)",
...
if uploaded_files:
    ...
    st.info("👆 อัปโหลดภาพชายฝั่งอย่างน้อย 2 ภาพเพื่อเริ่มต้น (แนะนำให้เรียงตามลำดับเวลา)")
...
def run_pipeline(configs, blur_kernel, land_is_bright, edge_method, canny_low, canny_high, max_dim,
    ...
    for cfg in configs:
        ...
        try:
            ...
            errors.append(f"⚠️ ไม่สามารถอ่านไฟล์ภาพ '{up_file.name}' ได้ ({exc}) — ข้ามภาพนี้")
            continue
        ...
        if coast_result is None:
            errors.append(
                f"⚠️ ตรวจไม่พบแนวชายฝั่งในภาพ '{up_file.name}' "
                f"ลองปรับค่า Blur/Threshold/Denoise ดู — ข้ามภาพนี้"
            )
            continue
        ...
        if coastline_length_px <= 0 or int(np.count_nonzero(edge_img)) == 0:
            errors.append(
                f"⚠️ แนวชายฝั่งในภาพ '{up_file.name}' หายไปหลังลด Noise "
                f"ลองลดค่า Denoise Kernel Size ดู — ข้ามภาพนี้"
            )
            continue

        fd_result = box_counting_fd(edge_img)
        if fd_result is None:
            errors.append(f"⚠️ ภาพ '{up_file.name}' เล็ก/สม่ำเสมอเกินไปสำหรับการคำนวณ Box-Counting — ข้ามภาพนี้")
            continue
...
if analyze_clicked:
    try:
        with st.spinner("กำลังประมวลผลภาพ — สกัดแนวชายฝั่งและคำนวณ Fractal Dimension..."):
        ...
        st.error(f"เกิดข้อผิดพลาดที่ไม่คาดคิดระหว่างการประมวลผล: {exc}")
...
if st.session_state.get("processed"):
    ...
    if len(results) == 0:
        st.error("ไม่มีภาพใดประมวลผลสำเร็จเลย กรุณาตรวจสอบข้อมูลที่กรอกแล้วลองใหม่อีกครั้ง")
        st.stop()
    ...
    # --------------------------------------------------------------------
    st.markdown("---")
    st.subheader("🖼️ 2. ตัวอย่างผลการประมวลผลภาพ")

    for r in results:
        resize_note = ""
        if r["was_resized"]:
            resize_note = (
                f", ย่อขนาดอัตโนมัติจาก {r['original_shape'][1]}x{r['original_shape'][0]} "
                f"เป็น {r['processed_shape'][1]}x{r['processed_shape'][0]}"
            )
        st.markdown(
            f"**{r['file_name']}** — {r['date'].isoformat()} "
            f"(สเกล: {r['scale_m_per_px']:.4f} ม./px{resize_note})"
        )
        ...
        with c2:
            st.image(
                r["cleaned_mask"], caption=f"มาสก์ขาวดำ — ผ่านการลด Noise (Otsu={r['otsu_val']:.1f})",
                use_container_width=True,
            )
        with c3:
            st.image(r["edge_img"], caption="แนวชายฝั่งที่สกัดได้ (เฉพาะเส้นหลัก)", use_container_width=True)
        st.caption(
    ...
    for idx, r in enumerate(results):
        ...
        if idx == 0:
            ...
            row["Status"] = "ข้อมูลฐาน (จุดเริ่มต้น)"
        else:
            prev = results[idx - 1]
    ...
    display_summary = summary_df.rename(columns={
        ...
        "FD": "FD",
        "Distance_Change_m": "ระยะเปลี่ยนแปลง (ม.)",
        "Status": "สถานะ",
    })
    st.dataframe(
    ...
    # --------------------------------------------------------------------
    st.markdown("---")
    st.subheader("📐 4. การตรวจสอบ Box-Counting (Log-Log Regression)")
    ...
    fig_diag.add_trace(go.Scatter(
        ...
        name=f"เส้น Regression (FD = slope = {sel['fd']:.4f})",
        line=dict(color="#d62728", dash="dash"),
    ))
    fig_diag.update_layout(
        title=f"กราฟ Log-Log ของ Box-Counting — {sel['file_name']} (R² = {sel['fd_r_squared']:.4f})",
    ...
    st.dataframe(
        pd.DataFrame({
            "ขนาดกล่อง r (px)": sel["box_sizes"],
            "จำนวนกล่อง N(r)": [int(c) for c in sel["counts"]],
        }),
        use_container_width=True,
    )
    ...
    # --------------------------------------------------------------------
    st.markdown("---")
    st.subheader("📈 5. การกัดเซาะ vs การงอก — การเปลี่ยนแปลงรายช่วงเวลา")

    if len(results) < MIN_IMAGES_FOR_TRENDS:
        st.info(f"อัปโหลดภาพอย่างน้อย {MIN_IMAGES_FOR_TRENDS} ภาพที่มีวันที่ถูกต้อง เพื่อคำนวณแนวโน้มการกัดเซาะ/งอก")
    else:
        interval_labels = []
        interval_distances = []
        ...
        fig_bar.update_layout(
            title="การเปลี่ยนแปลงชายฝั่งรายช่วงเวลา (เขียว = งอก, แดง = กัดเซาะ)",
            xaxis_title="ช่วงเวลา", yaxis_title="ระยะเปลี่ยนแปลง (ม.)", height=450,
        )
        ...
        for idx in range(1, len(results)):
            ...
            if status == "กัดเซาะ":
                st.error(f"📉 {d_label}: **กัดเซาะ {abs(v):.3f} เมตร**")
            elif status == "งอก":
                st.success(f"📈 {d_label}: **งอก {abs(v):.3f} เมตร**")
            else:
                st.info(f"➖ {d_label}: **คงที่** (Δ = {v:+.3f} ม.)")

    # --------------------------------------------------------------------
    # 6. FRACTAL DIMENSION TREND & YEAR-BY-YEAR FORECAST
    # --------------------------------------------------------------------
    st.markdown("---")
    st.subheader("🔮 6. แนวโน้ม Fractal Dimension และการพยากรณ์ (รายปี)")

    if len(results) < MIN_IMAGES_FOR_TRENDS:
        st.info(f"อัปโหลดภาพอย่างน้อย {MIN_IMAGES_FOR_TRENDS} ภาพ เพื่อเปิดใช้งานการพยากรณ์")
        ...
        if len(results) < 3:
            st.caption("⚠️ การพยากรณ์นี้อ้างอิงจากข้อมูลจำนวนน้อย (น้อยกว่า 3 ภาพ) — โปรดพิจารณาผลลัพธ์ด้วยความระมัดระวัง")
        ...
        with col_b:
            st.metric(
                ...
                f"{last_row['FD']:.4f}",
                delta=f"รวม {last_row['Cumulative_Distance_m']:+.3f} ม. ({last_row['Status']})",
            )

        st.caption(
            f"โมเดลแนวโน้ม: FD = {model.coef_[0]:.6f} × ปี + {model.intercept_:.6f}   (R² = {r2_trend:.4f})"
        )
        ...
        fig_forecast.add_trace(go.Scatter(
            ...
            name="FD ย้อนหลัง", line=dict(color="#1f77b4"), marker=dict(size=9),
        ))
        trend_x = [float(years_arr.min()), float(years_arr.max())]
        fig_forecast.add_trace(go.Scatter(
            x=trend_x, y=model.predict(np.array(trend_x).reshape(-1, 1)),
            mode="lines", name="เส้นแนวโน้ม", line=dict(color="gray", dash="dot"),
        ))
        ...
        fig_forecast.add_trace(go.Scatter(
            ...
            mode="lines+markers", name=f"พยากรณ์ (+1 ถึง +{int(horizon)} ปี)",
            line=dict(color="#d62728", dash="dash"), marker=dict(size=9, symbol="diamond"),
        ))
        fig_forecast.update_layout(
            title="แนวโน้มและการพยากรณ์ Fractal Dimension",
            xaxis_title="ปี", yaxis_title="Fractal Dimension (FD)", height=460,
        )
    ...
    # --------------------------------------------------------------------
    st.markdown("---")
    st.subheader("💾 7. ดาวน์โหลดรายงานฉบับเต็ม")
    ...
    export_df = export_df.rename(columns={
        "Date": "วันที่",
        "FD": "ค่า_FD",
        "Distance_Change_m": "ระยะกัดเซาะ_งอก_เมตร",
        "Cumulative_Distance_m": "ระยะสะสม_เมตร",
        "Status": "สถานะ",
        "Type": "ประเภท",
    })
    export_df = export_df[
        ["วันที่", "ค่า_FD", "ระยะกัดเซาะ_งอก_เมตร", "ระยะสะสม_เมตร", "สถานะ", "ประเภท"]
    ]
    ...
    st.markdown("---")
    st.caption(
        "พัฒนาด้วย Streamlit, OpenCV, NumPy, scikit-learn และ Plotly "
        "ผลลัพธ์เป็นการประมาณค่าตามหลักฮิวริสติก เหมาะสำหรับการวิเคราะห์เชิงสำรวจ/การศึกษาเท่านั้น"
    )
